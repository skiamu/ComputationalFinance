function [price] = FDLogPriceUpWind( S0,K,r,T,N,M,sigma,optionType,NumMethod,theta,...
	barrierType,barrier)
...
%% matrix definition
e = ones([N-1 1]);
csi = abs(r - sigma^2/2) * dx/2;
switch NumMethod
	case 'explicit'
		alpha_tilde = (-(r - sigma^2/2) / (2*dx) + (sigma^2 + 2*csi)/(2*dx^2));
		beta_tilde = 1 / dt + (-(sigma^2 + 2*csi)/dx^2 - r);
		gamma_tilde = ((r - sigma^2/2) / (2*dx) + (sigma^2 + 2*csi)/(2*dx^2));
		M2 = spdiags([alpha_tilde*e beta_tilde*e gamma_tilde*e],-1:1,N-1,N-1);
		M1 = spdiags((1/dt)*e,0,N-1,N-1);
	case 'implicit'
		alpha = -(-(r - sigma^2/2) / (2*dx) + (sigma^2 + 2*csi)/(2*dx^2));
		beta = 1 / dt - (-(sigma^2 + 2*csi)/dx^2 - r);
		gamma = -((r - sigma^2/2) / (2*dx) + (sigma^2 + 2*csi)/(2*dx^2));
